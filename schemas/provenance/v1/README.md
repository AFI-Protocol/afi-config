...
| AnalystInputEnvelope v1 | `analyst-input-envelope.schema.json` | Strict wrapper around an intentionally opaque `strategyLocalView`. The view must be declared (`strategyViewType` and/or `enrichedViewSchemaRef`) and only participates in hashing via the explicit `strategyLocalViewHash` pin. Per-lane provenance now lives in the governed `afi.provider-invocation-proof.v1` on the v3 evidence record (EV3-GOV D-EV3-8(2) removed the envelope's dormant per-lane draft member; the D2 draft family is eight artifact kinds). |
