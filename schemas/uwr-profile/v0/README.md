...
- **Qualification is recorded, not wired** — `minDecayScoreThreshold 0.5` is a testnet-provisional value (UP-9); no `IValidatorScorer` composition exists or is authorized here. The profile's `challengeWindowDurationHours` field is VOID per CHR-GOV D-CHR-3; the document and its schema stay byte-frozen under UP-5 and the field is removed on the profile v0.2 re-version.
